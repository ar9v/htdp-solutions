...
;;; Equip the program from exercise 222 with a `stop-when` clause. The game ends when one
;;; of the columns contains enough blocks to "touch" the top of the canvas.
...
; game-over?: Tetris -> Boolean
; True if any column in the game has touched the top of the canvas
(check-expect (game-over? tetris0) #false)
(check-expect
 (game-over? (make-tetris block-dropping (create-block-column 2 SIZE)))
 #true)
(define (game-over? t)
  (landscape-touches-top? (tetris-landscape t)))

; landscape-touches-top?: Landscape -> Boolean
; True if any block in the landscape has a y-coordinate of 0
(check-expect (landscape-touches-top? landscape0) #false)
(check-expect (landscape-touches-top? stacked-landscape) #false)
(check-expect (landscape-touches-top? (create-block-column 1 SIZE)) #true)
(define (landscape-touches-top? l)
  (and (not (empty? l))
       (or (= (block-y (first l)) 0)
           (landscape-touches-top? (rest l)))))

; create-block-column: Number Number -> Landscape
; creates a landscape made out of stacked blocks in `x`, from y-coordinates [0, n)
(check-expect (create-block-column 0 0) '())
(check-expect (create-block-column 0 3)
              (list (make-block 0 2) (make-block 0 1) (make-block 0 0)))
(define (create-block-column x n)
  (cond [(zero? n) '()]
        [else (cons (make-block x (- n 1))
                    (create-block-column x (- n 1)))]))
