...
;;; The wrapping of `cond` with `(beside/align 'center BT ...)` may surprise you. Edit the
;;; function definition so that the wrap-around appears once in each clause. Why are you
;;; confident that your change works? Which version do you prefer?
;;;
;;; A:
;;; We can be confident the change works because we can flesh out the steps in which that
;;; expression is evaluated, and conclude that whether the cond or `bulletize` wraps, we
;;; wind up with a call to `bulletize` with one or the other expression as the argument.
;;;
;;; I personally prefer the original, because I feel that the `cond` branches
;;; wind up being distinct.
...
(define (render-item xitem)
  (local [(define content (first (xexpr-content xitem)))]
    (cond [(word? content) (bulletize (text (word-text content) SIZE 'black))]
