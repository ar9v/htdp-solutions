...
;;; Intead the text is cut off at the right margin. Modify your function `edit` from
;;; exercise 84 so that it ignores a keystroke if adding it to the end of the `pre` field
...
(define (edit ed ke)
  (cond [(key=? ke "left")
        ...
        [(and (1string? ke)
              (not (escaped? ke))
              (not (overflows? (editor-pre-append ed ke))))
         (editor-pre-append ed ke)]
        [else ed]))
...
(check-expect (overflows? (make-editor (replicate EDITOR-WIDTH "h") "")) #true)
(define (overflows? ed)
  (< EDITOR-WIDTH (image-width (render ed))))

; editor-pre-append: Editor KeyEvent -> Editor
; Given an Editor `e` and a KeyEvent `ke`, produce an editor which is the result of
; appending `ke` to `e`'s `pre` field
(check-expect (editor-pre-append empty-ed "a") (make-editor "a" ""))
(check-expect (editor-pre-append cursor-in-mid "w") (make-editor "hellow" " world"))
(define (editor-pre-append ed ke)
