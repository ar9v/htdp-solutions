...
;;; Chapter 12.7 spells out a game of space war. In the basic version, a UFO descends and
;;; a player defends with a tank. One additional suggestion is to equip the UFO with
;;; charges that it can drop at the tank; the tank is destroyed if a charge comes close
;;; enough.
;;;
;;; Inspect the code of your project for places where it can benefit from existing
;;; abstraction, that is, processing lists of shots or charges.
;;;
;;; Once you have simplified the code with the use of existing abstractions, look for
;;; opportunities to create abstractions. Consider moving lists of objects as one example
;;; where abstraction may pay off.
...
(define (beside/list imgs) (foldr beside empty-image imgs))
...
(define (between-3-and-7? n) (and (<= 3 n) (< n 7)))
...
(define (render-missiles ms img)
  (local [(define (render-missile m curr-img)
            (place-image MISSILE (posn-x m) (posn-y m) curr-img))]
    (foldr render-missile img (missiles-posns ms))))
...
(define (render-ufo u img) (place-image UFO (ufo-x u) (ufo-y u) img))
...
(define (render-tank t img)
  (place-image TANK (tank-x t) (- WORLD-HEIGHT (/ (image-height TANK) 2)) img))
...
(define (update-missiles-posns posns)
  (local [(define (update-missile-posn p)
            (make-posn (posn-x p) (- (posn-y p) MISSILE-Y-DELTA)))
          (define (in-canvas? p) (<= 0 (posn-y p)))]
    (filter in-canvas? (map update-missile-posn posns))))
...
(define (any-missile-hits? ps u)
  (local [(define (within-ufo-hitbox? p)
            (< (distance p (make-posn (ufo-x u) (ufo-y u))) UFO-HITBOX))]
    (ormap within-ufo-hitbox? ps)))
