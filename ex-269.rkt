...
; eliminate-expensive: Number [List-of IR] -> [List-of IR]
; Picks IRs from `irs` which have a sales price lower than `ua`
...
(define (eliminate-expensive ua irs)
  (local ((define (inexpensive? ir) (< (ir-sales-price ir) ua)))
    (filter inexpensive? irs)))
...
(define (recall ty irs)
  (local ((define (named-ty? ir) (not (string=? (ir-name ir) ty))))
    (filter named-ty? irs)))
...
(define (selection los1 los2)
  (local ((define (in-los1? s) (member? s los1)))
    (filter in-los1? los2)))
