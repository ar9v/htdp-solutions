...
;;
;; That is, assuming that you have solved exercise 39, define the constants BACKGROUND
;; and Y-CAR. Then assemble all the function definitions, including their tests. When
...
;;; Also add a clause to the `big-bang` expression that stops the animation when the car
...
;;; We add BODY-WIDTH (vs. (/ BODY-WIDTH 2)) for aesthetic reasons: this way, the car
;;; has enough leeway to not stop right at the border of BACKGROUND, messing up the
;;; outline.
...
(check-expect (render 1) (place-image CAR 1 Y-CAR BACKGROUND))
(check-expect (render 30) (place-image CAR 30 Y-CAR BACKGROUND))
(check-expect (render -20) (place-image CAR -20 Y-CAR BACKGROUND))
(define (render ws)
