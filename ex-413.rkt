     ...
     (local [(define (normalize i)
               (match i
                 [(inex m s e)
                  (if (< m 99)
                      i
                      (normalize (make-inex (round (/ m 10)) s (abs (add1 (* s e))))))]))
       ...
       (cond [(inex< normalized MIN-POSITIVE)
              (error "Error: the multiplication underflowed!")]
             [(inex> normalized MAX-POSITIVE)
              (error "Error: the multiplication overflowed!")]
             [else normalized]))]))
